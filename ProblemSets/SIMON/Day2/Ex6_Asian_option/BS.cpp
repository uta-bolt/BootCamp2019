...
double asian_monte_carlo_call_price(const int& num_sims, const double& S, const double& K, const double& r, const double& v, const double& T) {
  double S_adjust = S * exp((tnew-told)*(r-0.5*v*v)); //this is what he calls S(T)
  double S_cur = 0.0;
  double payoff_sum = 0.0;
  ...
  for (int i=0; i<num_sims; i++) {
    ...
    for (int j=0; j<num_trans;j++){
      tnew=j+1;
      double Zij= gaussian_box_muller();
      S_cur += S_adjust * exp(sqrt(v*v*(tnew-told))*gauss_bm); //gauss_bm is Z_I
      told=tnew;
      }
    double S_bar=<double> S_cur/<double> m;
...
int main(int argc, char **argv) {
  ...
  int num_trans=1000; //Number of transition paths
  ...
  double call_asian= asian_monte_carlo_call_price(num_sims, S, K, r, v, T)
  ...
  std::cout << "Asian Price:      " << call_asian << std::endl;
  return 0;
}
